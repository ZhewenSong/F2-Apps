...
import org.apache.tez.examples.Anagram.TokenProcessor;
...
/**
 * Simple example that extends the Anagram example to show a chain of processing.
 * The example extends Anagram by sorting the words by their count.
 */
public class OrderedAnagram extends TezExampleBase {
  
  private static String INPUT = Anagram.INPUT;
  private static String OUTPUT = Anagram.OUTPUT;
  private static String TOKENIZER = Anagram.TOKENIZER;
  private static String SUMMATION = Anagram.SUMMATION;
  private static String SORTER = "Sorter";
  private static final Logger LOG = LoggerFactory.getLogger(OrderedAnagram.class);

  /*
   * SumProcessor similar to Anagram except that it writes the count as key and the 
  ...
  public static class SumProcessor extends SimpleProcessor {
    ...
    public void run() throws Exception {
      ...
      LOG.info("F2DATA: input_ready," + System.currentTimeMillis() + "," + SUMMATION);
      while (kvReader.next()) {
        Text word = (Text) kvReader.getCurrentKey();
        //LOG.info("Arjun - In sum processor - Word " + word.toString());
        int sum = 0;
        String string = "";
        for (Object value : kvReader.getCurrentValues()) {
          //LOG.info("Arjun - In sum processor - partial sum: " + sum);
            sum++;
            string += ((Text) value).toString() + " ";

//          kvWriter.write(new IntWritable(1), word);
        }
        // write the sum as the key and the word as the value
        //LOG.info("Arjun - In sum processor - Sum " + sum);
        //kvWriter.write(new IntWritable(1/*sum*/), word);
        kvWriter.write(new IntWritable(sum), new Text(string));
      }
      long time_finish = System.currentTimeMillis();
      LOG.info("F2DATA: task_done," + time_finish + "," + SUMMATION);
      LOG.info("F2DATA: task_time," + (time_finish - time_start) / 1000 + "," + SUMMATION);
    }
  }
  ...
  public static class NoOpSorter extends SimpleMRProcessor {
    ...
    public void run() throws Exception {
      ...
      long time_start = System.currentTimeMillis();
      LOG.info("F2DATA: input_ready," + System.currentTimeMillis() + "," + "SORTER");
      while (kvReader.next()) {
        Object sum = kvReader.getCurrentKey();
        for (Object word : kvReader.getCurrentValues()) {
          //LOG.info("Arjun - In NoOpSorter, word, count: " + word.toString() + ", " + sum );
          kvWriter.write(word, sum);
        }
      }
      long time_finish = System.currentTimeMillis();
      LOG.info("F2DATA: task_done," + time_finish + "," + "SORTER");
      LOG.info("F2DATA: task_time," + (time_finish - time_start) / 1000 + "," + "SORTER");
  ...
  public static DAG createDAG(TezConfiguration tezConf, String inputPath, String outputPath,
      int numPartitions, boolean disableSplitGrouping, String dagName) throws IOException {
          ...
          Resource resourceTokenizer = Resource.newInstance(4096, 1);
          Resource resourceSummation = Resource.newInstance(4096, 1);
          Resource resourceSorter = Resource.newInstance(4096, 1);
    ...
    Vertex tokenizerVertex = Vertex.create(TOKENIZER, ProcessorDescriptor.create(
        TokenProcessor.class.getName()), -1, resourceTokenizer);
    tokenizerVertex.addDataSource(INPUT, dataSource);

    // Use Text key and Text value to bring counts for each word in the same partition
    ...
    Vertex summationVertex = Vertex.create(SUMMATION, ProcessorDescriptor.create(
        SumProcessor.class.getName()), 4, resourceSummation);
    
    // Use Int key and Text value to bring all words with the same count in the same 
    ...
    Vertex sorterVertex = Vertex.create(SORTER, ProcessorDescriptor.create(
        NoOpSorter.class.getName()), 1, resourceSorter);
  ...
  protected int runJob(String[] args, TezConfiguration tezConf,
    ...
    DAG dag = createDAG(tezConf, args[0], args[1],
        ...
        "OrderedAnagram");
    LOG.info("Running OrderedAnagram");
    return runDag(dag, false, LOG);
  }

  public static void main(String[] args) throws Exception {
    int res = ToolRunner.run(new Configuration(), new OrderedAnagram(), args);
    System.exit(res);
  }
}
